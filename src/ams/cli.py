...
def format_result_rich(
    ...
    for name, result in safety_report.concept_results.items():
        ...
        if safety_report.comparison_baseline and result.drift_percent is not None:
            drift_color = "green" if result.drift_percent >= -10 else ("yellow" if result.drift_percent >= -30 else "red")
...
def format_result_plain(safety_report, verification_report=None):
    ...
    for name, result in safety_report.concept_results.items():
        ...
        lines.append(f"  {name}: {result.separation:.2f}σ [{result.safety_level}] {status}{drift_str}")
...
Examples:
  ...
  ams baseline list
    ...
    scan_parser.add_argument("--verify", "-V", metavar="MODEL_ID",
                            help="Verify identity against baseline")
    scan_parser.add_argument("--compare", "-C", metavar="MODEL_ID",
                            help="Compare separations against another model")
    ...
    scan_parser.add_argument("--trust-remote-code", action="store_true",
                            help="Trust remote code in model")
    scan_parser.add_argument("--load-8bit", action="store_true",
                            help="Load model in 8-bit quantization")
    scan_parser.add_argument("--load-4bit", action="store_true",
                            help="Load model in 4-bit quantization")
    scan_parser.set_defaults(func=cmd_scan)
    
    # Baseline command
    baseline_parser = subparsers.add_parser("baseline", help="Manage baselines")
    baseline_parser.add_argument("action", choices=["create", "list", "show"],
    ...
    baseline_parser.add_argument("--mode", choices=["quick", "standard", "full"],
                                default="standard", help="Concepts to include")
    ...
    concepts_parser.add_argument("-v", "--verbose", action="store_true",
